...
export const DropdownAssets = ({ isOpen, open }: Props) => {
  ...
  return (
    <div className="relative cursor-pointer">
      ...
      {
        isOpen && <BodyDropdown open={() => open(false)} reference={ref} />
      }
    </div>
  )
}
...
export const BodyDropdown = ({ open, reference }: { open: MouseEventHandler, reference: React.RefObject<HTMLDivElement> }) => {
        ...
        <div className="grid grid-cols-1">
          {
            datas.dataFeed.map((item, index) => {
              return (
                <div key={`data-feed-${index}`} className="grid grid-cols-[1fr_50px] px-3 py-2 text-sm hover:bg-orange-50/10">
